...
def get_math_agent(queue: Queue = None):
    ...
    question_writer = get_question_writer_tool(llm=gpt3, latex_llm=latex_llm, wrapper_llm=gpt4)
    question_solver = get_question_solver_tool(llm=gpt3, latex_llm=latex_llm, wrapper_llm=gpt4)
...
# chain = get_latex_chain(get_openai_llm(OPENAI_API_KEY), None, None)
